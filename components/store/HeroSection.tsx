      ...
      <div className="store-container">
        <div className="py-10 sm:py-14 lg:py-20">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <h1 className="mt-4 text-balance text-4xl font-semibold tracking-tight text-earth-900 sm:text-5xl lg:text-6xl">
              African &amp; Caribbean groceries,
              <br className="hidden sm:block" />
              <span className="text-brand-700"> delivered fast.</span>
            </h1>
            <p className="mx-auto mt-5 max-w-xl text-base text-earth-600 sm:text-lg">
          ...
          <div className="mt-10 flex flex-wrap items-center justify-center gap-2">
            ...
            {QUICK_LINKS.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="rounded-full border border-earth-200 bg-white px-3 py-1.5 text-xs font-medium text-earth-700 no-underline transition-colors duration-150 hover:border-earth-300 hover:bg-earth-50 hover:text-earth-900"
              >
                {link.label}
              </Link>
            ))}
