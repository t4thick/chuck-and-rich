    ...
    () =>
      POPULAR.map((term) => (
        <button
          key={term}
          type="button"
          className="rounded-full bg-earth-100 px-2.5 py-1 text-[11px] font-medium text-earth-700 transition-colors hover:bg-earth-200"
          onClick={() => {
            setQ(term)
            submit(term)
          }}
      ...
      {showDropdown && (
        ...
        >
          {!trimmed ? (
            ...
            <div className="flex items-center gap-2 px-4 py-3 text-sm text-earth-500">
